#!/usr/bin/env Rscript
# ============================================================================
# Step_080: Plotting Model Results (ATAC-only, Automated)
# ============================================================================
...
# This is the ATAC-only version (uses ATAC-derived HVG features / peaks).
#
...
cat("\n")
cat("=", rep("=", 70), "\n", sep = "")
cat("STEP 080: Plotting Model Results (ATAC-only)\n")
cat("=", rep("=", 70), "\n\n", sep = "")
  ...
  if (nrow(metrics) > 0) {
    ...
    p_r2 <- r2_clean %>%
      ...
      labs(title = expression(bold(R^2 ~ "Distribution by Model (Test Set)")),
           subtitle = sprintf("%s | %s | ATAC-only (HVG features)", SAMPLE_NAME, DIMRED_METHOD_SUFFIX),
    ...
    combined_metrics <- (p_r2 | p_rmse) / p_spearman +
      plot_annotation(
        title = "Model Performance Comparison (ATAC-only)",
    ...
    if (nrow(coeffs_df) > 0) {
      ...
      p_coef_dist <- coeffs_df_hist %>%
        ...
        labs(title = "Coefficient/Importance Distribution (Linear + RF)",
             subtitle = sprintf("%s | Log-scaled x-axis | ATAC-only features", SAMPLE_NAME),
...
cat("\n")
cat("=", rep("=", 70), "\n", sep = "")
cat("STEP 080 COMPLETE - PUBLICATION-READY PLOTS (ATAC-only)\n")
cat("=", rep("=", 70), "\n\n", sep = "")
...
cat("  - ATAC-only (HVG features / peaks)\n")
